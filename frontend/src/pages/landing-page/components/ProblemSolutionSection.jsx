...
const ProblemSolutionSection = () => {
  ...
  return (
    <section id="problem-solution" className="py-20 bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            India generates millions of tons of waste daily while people struggle with expensive new products. 
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            <div className="relative">
              <h3 className="text-2xl font-bold text-destructive mb-6">The Wasteful Reality</h3>
                  ...
                  {wasteStats?.map((stat, index) => (
                    <div key={index} className="flex items-center space-x-4">
                      <div className="flex items-center justify-center w-12 h-12 bg-destructive/10 rounded-full">
                        <Icon name={stat?.icon} size={24} className="text-destructive" />
                      </div>
                      <div>
                    ...
                    {[...Array(8)]?.map((_, i) => (
                      <div
                        ...
                        className={`bg-destructive/60 rounded-t-lg transition-all duration-1000 ${
                          isVisible ? `h-${Math.floor(Math.random() * 20) + 10}` : 'h-2'
                        }`}
                        style={{
                          width: '12px',
                          height: isVisible ? `${Math.floor(Math.random() * 80) + 20}px` : '8px',
          ...
          <div className="space-y-8">
            <div className="relative">
              <h3 className="text-2xl font-bold text-primary mb-6">Our Circular Solution</h3>
              ...
              <div className="relative bg-card rounded-2xl p-8 shadow-eco-card border border-primary/20">
                <div className="relative">
                  ...
                  <div className="relative w-64 h-64 mx-auto">
                    ...
                    <div className="absolute inset-4 border-2 border-dashed border-primary/40 rounded-full animate-spin" style={{ animationDuration: '20s' }}></div>
                    
                    {/* Lifecycle Steps */}
                    {[
                      { icon: "ShoppingBag", label: "Buy", position: "top-0 left-1/2 -translate-x-1/2 -translate-y-1/2" },
                      { icon: "Heart", label: "Use", position: "top-1/2 right-0 translate-x-1/2 -translate-y-1/2" },
                      { icon: "RefreshCw", label: "Sell", position: "bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2" },
                      { icon: "Repeat", label: "Reuse", position: "top-1/2 left-0 -translate-x-1/2 -translate-y-1/2" }
                    ]?.map((step, index) => (
                      <div key={index} className={`absolute ${step?.position} z-10`}>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                {circularBenefits?.map((benefit, index) => (
                  <div key={index} className="bg-card rounded-lg p-4 shadow-eco-card border border-border hover:shadow-eco-cta transition-eco">
        ...
        <div className="mt-16 bg-card rounded-2xl p-8 shadow-eco-card">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold text-foreground mb-2">The Impact Difference</h3>
            <p className="text-muted-foreground">See how choosing second-hand creates positive change</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h4 className="font-semibold text-foreground mb-2">Traditional Shopping</h4>
